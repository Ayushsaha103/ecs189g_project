...
class Method_Classification_LSTM(method, nn.Module):
    data = None
    ...
    def __init__(self, mName, mDescription, save_dir, vocab_size: int, embedding_size: int, hidden_size: int, num_lstm_layers:int,  output_size: int,
        ...
        # LSTM
        self.rnn = nn.LSTM(embedding_size, hidden_size, num_layers=num_lstm_layers, dropout=0.35, batch_first=True,
                           bidirectional=True)
    ...
    def train(self, X, y, test_data: None):
        ...
        for epoch in range(self.max_epoch):
            ...
            for batch_index in range(0, X.shape[0], self.batch_size):
                ...
                y_true = torch.tensor(batch_y, dtype=torch.float).to(self.device).view(-1, 1)
